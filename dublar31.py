...
def split_long_segments(segments, maxdur):
    ...
    for s in segments:
        ...
        for i, piece in enumerate(pieces):
            ...
            out.append({"start": cur, "end": nxt, "text_trad": piece}); cur = nxt
...
def sync_fit(p, target, workdir, sr, tol, maxstretch):
    """
    Ajusta o áudio do trecho para caber exatamente no tempo 'target'.
    Estratégia:
      - Se a diferença for pequena (<= tolerance): não usa atempo. Apenas:
         * se sobrar: atrim para target exato
         * se faltar: apad + atrim para target exato
      - Se a diferença for grande: usa atempo (limitado por maxstretch) e SEMPRE finaliza com atrim.
    """
    ...
    # caso 1: dentro da tolerância → sem atempo, só pad/corte + atrim
    if within_tol:
        out = Path(workdir, p.name.replace(".wav", "_fit.wav"))
        if diff >= 0:
            # falta tempo: completa micro-silêncio e trava em target
            fchain = f"apad=pad_dur={diff:.6f},atrim=duration={target:.6f}"
        else:
            # sobra tempo: corta exatamente em target
            fchain = f"atrim=duration={target:.6f}"
        sh(["ffmpeg","-y","-i", p.name, "-af", fchain, "-ar", str(sr), "-ac","1", out.name], cwd=workdir)
        return out

    # caso 2: fora da tolerância → atempo (limitado) + atrim para cravar
    ratio = (cur / target) if target > 0 else 1.0
    ratio = max(min(ratio, maxstretch), 1.0 / maxstretch)  # limita ajuste
    f_atempo = atempo_chain(ratio)  # cadeia 0.5–2.0 por etapa
    out = Path(workdir, p.name.replace(".wav", "_fit.wav"))
    # aplica atempo e SEMPRE finaliza com atrim para target exato
...
def main():
    ...
    ap.add_argument("--sync", choices=["none","fit","pad","smart"], default="none")
    ...
    ap.add_argument("--fade", type=float, default=0.02, help="duração do fade in/out em segundos (0 = desativa)")

    ap.add_argument("--preserve-gaps", action="store_true", help="insere silêncio entre segmentos conforme timestamps")
    ap.add_argument("--gap-min", type=float, default=0.20, help="gap mínimo em segundos para inserir silêncio")
    ...
    # pasta de saída e nome do arquivo final
    outdir = Path("dublado")
